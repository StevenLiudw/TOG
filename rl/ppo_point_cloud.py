...
def main():
    ...
    rollout_steps = 3200
    model_dir = f'saved_models/{args.exp_name}'
    ...
    if args.eval:
        record_dir = f"videos/{args.exp_name}/eval"
    else:
        record_dir = f"videos/{args.exp_name}"
    env_fn = partial(
        make_env,
        env_id,
    ...
    if args.eval:
        ...
        if model_path is None:
            model_path = osp.join(model_dir, "best_model.zip")
        ...
        model.learn(total_timesteps, callback=[eval_callback],tb_log_name=args.exp_name)
